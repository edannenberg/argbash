...
_help_command_description="foo command description goes here"

# ARGBASH_WRAP([opt-global])
# ARG_OPTIONAL_SINGLE([foo-option],[o],[A option with short and long flags and default],[boo])
# ARG_POSITIONAL_SINGLE([foo-arg],[Positional arg description for the foo command],[])
# ARG_OPTIONAL_ACTION([help],[h],[Prints help],[show_help])
# ARGBASH_SET_INDENT([    ])
# ARGBASH_GO()
...
print_help ()
{
    printf 'Usage: %s [--(no-)print] [-o|--foo-option <arg>] [-h|--help] <foo-arg>\n' "$0"
    printf "\t%s\n" "<foo-arg>: Positional arg description for the foo command"
    printf "\t%s\n" "--print,--no-print: A boolean option with long flag (and implicit default: off) (off by default)"
    printf "\t%s\n" "-o,--foo-option: A option with short and long flags and default (default: '"boo"')"
    printf "\t%s\n" "-h,--help: Prints help"
}
...
do
    ...
    case "$_key" in
        --no-print|--print)
            ...
            ;;
        -o*|--foo-option|--foo-option=*)
            _val="${_key##--foo-option=}"
            _val2="${_key##-o}"
            if test "$_val" = "$_key"
            then
...
_positional_names=('_arg_foo_arg' )
